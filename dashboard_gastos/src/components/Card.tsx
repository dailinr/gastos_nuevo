...
export const Card = ({ amount, label, svg, color, type }: cardProps) => {
  ...
  const classCategory = 'flex flex-col items-center overflow-hidden h-auto  w-fit';
  ...
  return (
    <div className={type === 'card' ?  classCard : classCategory } >
      ...
      <div className={`${type === 'card' ? "md:text-left " : "" } text-center flex-grow`}>
        <div className={`${type === 'card' ? "md:justify-end md:text-2xl font-semibold " : ""} flex text-center text-[14px]  break-words overflow-wrap-break-word`}>${amount}</div>
